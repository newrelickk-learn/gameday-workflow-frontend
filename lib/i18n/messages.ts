/**
 * 画面文言の辞書。
 *
 * 英語で演習を受けられるようにするため、画面の文言はここに集約する。
 * URLは言語で分けず(既存リンクやランブックのURLを壊さないため)、ヘッダーのトグルで
 * 即時に切り替える方式にしている。選択はlocalStorageに保存する。
 */

export type Locale = 'ja' | 'en';

export const LOCALES: Locale[] = ['ja', 'en'];

const ja = {
  common: {
    appTitle: 'ワークフローGameday',
    today: '今の日付',
    logout: 'ログアウト',
    back: '戻る',
    close: '閉じる',
    cancel: 'キャンセル',
    submit: '申請する',
    detail: '詳細を見る',
    loading: '読み込み中...',
    language: '言語',
    japanese: '日本語',
    english: 'English',
  },
  login: {
    title: 'ログイン',
    email: 'メールアドレス',
    password: 'パスワード',
    submit: 'ログイン',
    impactedPod: '問題のあるPod名',
    impactedPodHelper:
      'ログインはgameday-workflow-userサービスで行っています。APMのSummary画面の下の方にInfrastructureの情報があるので確認してください。\'pod: \'の後の文字列をコピペしましょう',
    failed: 'ログインに失敗しました',
    podSaturated:
      '現在サーバーが高負荷のためログインできません。New Relicで継続的にCPU % が50以上となっているPodを確認し、Pod名を入力してください',
    stubMode: 'スタブモード: 任意のメールアドレスとパスワードでログインできます',
    welcome: 'ワークフロー管理システムへようこそ',
  },
  dashboard: {
    applications: '申請一覧',
    applicationsDescription: '自分が提出した申請の一覧を確認できます',
    approvals: '承認待ち',
    approvalsDescription: '自分の承認が必要な申請を確認できます',
    companyApplications: '申請書一覧',
    companyApplicationsDescription: '会社全体の申請を確認できます',
    approved: '承認済み一覧',
    approvedDescription: '承認が完了した申請を確認できます',
    statistics: '統計',
    statisticsDescription: '申請の統計情報を確認できます',
    manual: 'マニュアル',
    manualDescription: '申請の書き方を確認できます',
    notifications: '通知一覧',
    notificationsDescription: '自分宛の通知を確認できます',
    hr: '人事部',
    hrDescription: '自社ユーザーの直属の上長を編集',
    heading: 'ダッシュボード',
    totalApplications: '総申請数',
    pendingApprovals: '承認待ちの申請',
    companyApproved: '自社の承認済みの申請',
    manualDetail: '各種申請の書き方や承認フローの説明',
    loadFailed: 'データの取得に失敗しました',
  },
  applications: {
    newTitle: '新規申請',
    type: '申請タイプ',
    selectPlaceholder: '選択してください',
    typeBusinessTrip: '出張申請',
    typeVacation: '有給休暇申請',
    typeExpense: '経費申請',
    typePromotion: 'プロモーション申請',
    title: 'タイトル',
    titlePlaceholder: '申請のタイトルを入力してください',
    amount: '金額',
    amountPlaceholder: '金額を入力してください',
    departure: '出発地',
    arrival: '到着地',
    estimatedCost: '概算出張費',
    estimating: '算出中...',
    estimateHint: '出発地・到着地を選択すると自動的に算出されます',
    estimateDiagnosisTitle: '概算出張費の取得に失敗する原因を診断する',
    estimateFailed:
      '旅費サービスとの接続がタイムアウトし、概算費用の取得に失敗しました。しばらくしてから再度お試しください。',
    startDate: '開始日',
    endDate: '終了日',
    days: '日数',
    daysPlaceholder: '日数を入力してください',
    daysUnit: '日',
    daysAuto: '自動計算: {days}日',
    daysAutoHint: '開始日と終了日を入力すると自動計算されます',
    dateFormatHint: '形式: YYYY-MM-DD',
    departureIn: '出発日まであと{days}日です。',
    departurePast: '出発日が既に過去の日付になっています。',
    twoWeekRule:
      ' 社内規定の「2週間前ルール」（出発日の{days}日前までの申請）を満たしていない可能性があります。ご注意ください。',
    description: '説明',
    descriptionPlaceholder: '申請の詳細を入力してください',
    descriptionAiHint:
      '出張の目的・訪問先・そこで行う業務内容を具体的に記入してください（AIレビュアーが確認します）',
    toConfirm: '確認画面へ',
    confirmTitle: '申請内容の確認',
    confirmBody: '以下の内容で申請します。内容をご確認の上、申請ボタンを押してください。',
    submitting: '申請中...',
    created: '申請が正常に作成されました',
    errorAmount: '金額を正しく入力してください',
    errorDates: '開始日と終了日を入力してください',
    errorDateOrder: '開始日は終了日より前である必要があります',
    errorDays: '日数を入力してください',
    errorNoLogin: 'ログイン情報が見つかりません。再度ログインしてください。',
    errorCreate: '申請できませんでした',
  },
  hr: {
    usersTitle: '人事部: ユーザー一覧',
    emptyUsers: 'ユーザーがいません',
    name: '名前',
    email: 'メールアドレス',
    role: '役職',
    department: '部署',
    manager: '直属の上長',
    actions: '操作',
    editManager: '上長を編集',
    editManagerTitle: '直属の上長を編集',
    backToList: '一覧へ戻る',
    unset: '未設定',
    save: '保存する',
    updated: '直属の上長を更新しました',
    loadUsersFailed: '自社ユーザー一覧の取得に失敗しました',
    loadUserFailed: 'ユーザー情報の取得に失敗しました',
    updateFailed: '直属の上長の更新に失敗しました',
  },
  workflow: {
    title: '承認フロー進捗',
    step: 'ステップ {n}',
    approver: '承認者',
    statusCompleted: '承認済み',
    statusCurrent: '承認待ち',
    statusRejected: '却下',
    statusNotStarted: '未開始',
    current: '現在の状況: ステップ {current} / {total} の承認待ちです',
    currentApprover: '（承認者: {name}）',
    allDone: '✓ すべての承認が完了しました。申請は承認されました。',
    rejected: '✗ 申請は却下されました。',
  },
  detail: {
    title: '申請詳細',
    number: '申請書番号: {number}',
    approve: '承認',
    reject: '却下',
    approveButton: '承認する',
    rejectButton: '却下する',
    rageClickHintPrefix: '承認ボタンで',
    rageClickTerm: 'Rage Click(レイジクリック/怒りクリック)',
    rageClickHintSuffix:
      'を発生させてください。（発生するとこのUIが変わります。リロードしつつ何度かお試しください。データ収集/分析に少し時間がかかります。）',
    applicationId: '申請ID',
    history: '承認履歴',
    historyStep: 'ステップ {n}',
    approverId: '承認者ID: {id}',
    comment: 'コメント',
    commentWithValue: 'コメント: {comment}',
    receipts: 'レシート画像（{count}枚）',
    notFound: '申請が見つかりませんでした',
    commentDialogTitle: '{action} - コメント入力',
    commentDialogBody: 'コメントを入力してください（任意）。確認画面に進みます。',
    commentLabel: 'コメント（任意）',
    commentPlaceholder: '承認または却下の理由を入力してください',
    confirmDialogTitle: '{action} - 確認',
    confirmApprove: 'この申請を承認しますか？',
    confirmReject: 'この申請を却下しますか？',
    processing: '処理中...',
    loadFailed: '申請詳細の取得に失敗しました',
    approvalFailed: '承認処理に失敗しました',
    promotionRestricted: 'プロモーション申請は上長・本部長のみ閲覧可能です',
  },
  misc: {
    scoreUpTo: 'CHAPTER {n} まで',
    scoreLive: 'LIVE / 全{n}クエスト',
    noTeams: 'まだスコアのあるチームがありません。',
    transferScores: '点数を転記',
    transferTitle: 'Dojoに点数を転記しますか?',
    transferBody: 'イベント {eventId} に、チーム{from}〜{to}のうち点数のある{n}チーム分を転記します。Dojo側の点数は上書きされます。',
    transferSubmit: '転記する',
    transferCancel: 'キャンセル',
    transferring: '転記中...',
    transferDone: '{n}チーム分を転記しました。',
    transferUnmatched: '{n}チームはこの開催に対応しなかったため転記していません。',
    transferFailed: '転記に失敗しました: {message}',
    progressBoard: 'チーム進捗ボード',
    ofChapters: '全{n}チャプター中',
    receiptAlt: 'レシート{n}',
    prevImage: '前の画像',
    nextImage: '次の画像',
    chatTitle: 'AI アシスタント',
    chatWelcome: '何かご質問がございましたら、お気軽にお尋ねください。',
    chatPlaceholder: 'メッセージを入力...',
    chatError: 'エラーが発生しました。もう一度お試しください。',
  },
  manual: {
    title: 'マニュアル',
    intro: '各種申請の書き方や承認フローについて説明しています。申請する前に該当する記事を確認してください。',
    backToList: 'マニュアル一覧',
  },
  lists: {
    applicationsTitle: '申請一覧',
    approvalsTitle: '承認一覧',
    toDashboard: 'ダッシュボード',
    newApplication: '新規申請',
    empty: '申請がありません',
    emptyApprovals: '承認待ちの申請がありません',
    number: '申請書番号',
    title: 'タイトル',
    applicationTitle: '申請タイトル',
    applicationId: '申請ID',
    type: 'タイプ',
    applicant: '申請者',
    amount: '金額',
    period: '期間',
    status: 'ステータス',
    step: '承認ステップ',
    stepValue: 'ステップ {current} / {total}',
    nextApprover: '次の承認者',
    createdAt: '作成日時',
    updatedAt: '更新日時',
    approvedDone: '承認完了',
    finalApproved: '最終承認済み',
    loadFailed: '申請一覧の取得に失敗しました',
    loadApprovalsFailed: '承認一覧の取得に失敗しました',
    loading: '読み込み中...',
    daysSuffix: '日',
    companyApplicationsTitle: '申請書一覧',
    approvedTitle: '承認済み一覧',
    emptyApproved: '承認済みの申請がありません',
    latestComment: '最新コメント',
    applicationType: '申請タイプ',
    loadCompanyFailed: '申請書一覧の取得に失敗しました',
    loadApprovedFailed: '承認済み一覧の取得に失敗しました',
    notificationsTitle: '通知一覧',
    backToDashboard: 'ダッシュボードへ戻る',
    emptyNotifications: '通知はまだありません',
    receivedAt: '受信日時',
    kind: '種類',
    subject: '件名',
    body: '内容',
    loadNotificationsFailed: '通知の取得に失敗しました',
    notificationApprovalRequest: '承認依頼',
    notificationApproved: '承認完了',
    notificationRejected: '却下',
    notificationSubmitted: '申請完了',
  },
  quiz: {
    submit: '送信する',
    searchOrType: '選択肢を検索、または直接入力',
    searchOrTypeEnter: '選択肢を検索、または直接入力してEnter',
    transaction360: {
      heading: 'gameday-workflow-frontendのCreateApplicationトランザクションを理解する',
      intro: '実際に申請して確認してみましょう。ヒント：CreateApplication /',
      introTail: 'を確認し、以下の4つの質問に回答してください（当てはまるものをすべて選んでください）。',
      correct: '正解です！CreateApplicationトランザクションの理解を深めることができました。',
      correctHint: '申請エラーが発生している場合はエラーの内容も確認して解決しましょう',
      q1: 'Q1. CreateApplicationというリクエストに関連しているアプリケーションを全て選びましょう',
      q1Label: '関連アプリケーション（複数選択可）',
      q2: 'Q2. Transaction-initiating sessionsのリストに表示されているのは、Browserエージェントで収集した操作履歴です。この操作の中には、必ずCreateApplication(申請)を行う操作が含まれていますか？（3つほど確認してみましょう）',
      q2Label: 'Yes / No（当てはまるものを選択）',
      q3: 'Q3. Supporting infrastructure entitiesのパネルで確認できる、正しい選択肢を全て選びましょう',
      q3Label: 'インフラに関する正しい記述（複数選択可）',
      q4: 'Q4. gameday-workflow-frontendから直接呼び出しているサービスを全て選んでください',
      q4Label: '直接呼び出しているサービス（複数選択可）',
    },
    rageClick: {
      heading: 'Rage Clickが検知されました。設問に答えてください。',
      intro: 'New Relicで調査した内容を元に、3つの質問に回答してください。',
      correct: '正解です！',
      q1: 'Q1. 検知に使用されたクエリはどれでしょうか',
      q1Label: 'NRQLクエリ（選択または直接入力）',
      q2: 'Q2. Rage Clickが検知された後、通知に成功した通知先はどれでしょうか',
      q2Label: '通知先（選択または直接入力）',
      q3: 'Q3. 通知の処理として一番近い内容を選択してください',
      q3Label: '通知の処理内容（選択または直接入力）',
    },
    slowList: {
      heading: 'この画面が遅い原因を診断する',
      intro:
        'New RelicのPerformance Risks Inboxを使いましょう。調査した内容を元に、3つの質問に回答してください。ヒント：gameday-workflow-application-approval',
      correct: '正解です！原因を特定できました。',
      q1: 'Q1. どんなパフォーマンス問題が起こっているか',
      q1Label: '問題の種類（選択または直接入力）',
      q2: 'Q2. 問題が発生しているテーブル（複数選択可）',
      q2Label: 'テーブル名（複数選択可）',
      q3: 'Q3. どんな改善方法が提案されているか (ヒント：Generate insightsをクリックして改善例を確認しよう)',
      q3Label: '改善方法（選択または直接入力）',
    },
  },
  status: {
    approved: '承認済み',
    rejected: '却下',
    pending: '承認待ち',
  },
  roles: {
    director: '本部長',
    accounting: '経理',
    manager: '上長',
    engineer: 'エンジニア',
    hr: '人事部',
  },
  missions: {
    heading: 'ミッション',
    hiddenHeading: '裏ミッション',
    intro:
      'パネルをクリックすると、そのミッションに挑戦できます。挑戦できるのは同時に1つだけで、クリアするまで他のミッションには移れません。',
    sealed: 'ミッション{n}\nに挑戦',
    sealedSide: 'ミッションに挑戦',
    cleared: 'Cleared!!',
    challenging: '挑戦中',
    challengers: '{n}チームが挑戦中',
    dialogTitleSealed: 'このミッションに挑戦しますか？',
    dialogSealedBody:
      'ミッションの内容は、挑戦を開始すると表示されます。挑戦できるのは同時に1つだけで、クリアするまで他のミッションには移れません。',
    challengingNotice: 'このミッションに挑戦中です。クリアすると次のミッションを選べるようになります。',
    startButton: 'このミッションに挑戦する',
    startFailed: '挑戦を開始できませんでした。',
    startFailedRetry: '挑戦を開始できませんでした。時間をおいて試してください。',
    reasonAnotherActive: '挑戦中のミッションをクリアするまで、他のミッションには挑戦できません。',
    reasonLocked: 'このミッションはまだ開放されていません。',
    reasonAlreadyCleared: 'このミッションはすでにクリア済みです。',
    reasonNotChallengeable: 'このミッションは挑戦の対象外です。',
    reasonUnknownCompany: 'チームの情報が取得できませんでした。ログインし直してください。',
  },
  diagnosis: {
    title: '原因を診断する',
    description: 'New Relicで調査した内容を元に、原因だと思う選択肢を選ぶか、直接入力してください。',
    inputLabel: '原因（選択または直接入力）',
    placeholder: '選択肢を検索、または直接入力',
    check: '判定する',
    correct: '正解です！原因を特定できました。',
    incorrect: '不正解でした。もう一度New Relicで調査し、選び直してください。',
    notChallenging:
      'このミッションにはまだ挑戦していないため、クリアとして記録されていません。ダッシュボードでこのミッションのパネルを開いてから、もう一度回答してください。',
    optionsFailed: '選択肢の取得に失敗しました',
    checkFailed: '判定に失敗しました',
  },
  runbook: {
    title: '暫定対応: 承認済み一覧の表示遅延',
    description:
      '一覧取得の読み込み方法を切り替えます。適用はアクセスした方が所属する会社にのみ行われ、他社には影響しません。適用状態は当日限りで、日次のメンテナンスでリセットされます。',
    applying: '適用しています…',
    applied:
      '暫定対応を適用しました。次回以降の承認済み一覧の取得からデータベース呼び出し回数が減ります。New Relicで適用前後の値を比較して、効果を確認してください。',
    already: 'この会社にはすでに暫定対応が適用されています。',
    blocked:
      '原因の切り分けが完了していないため、暫定対応は適用できません。先にランブックの「原因の切り分け」を完了してから、もう一度このページにアクセスしてください。',
    unauthenticated:
      '適用先の会社を特定できないため、ログインが必要です。ログインしてからもう一度アクセスしてください。',
    error: '暫定対応を適用できませんでした。時間をおいて、もう一度お試しください。',
    toLogin: 'ログイン画面へ',
    toDashboard: 'ダッシュボードへ',
    retry: 'もう一度試す',
  },
};

export type Messages = typeof ja;

const en: Messages = {
  common: {
    appTitle: 'Workflow GameDay',
    today: 'Current date',
    logout: 'Log out',
    back: 'Back',
    close: 'Close',
    cancel: 'Cancel',
    submit: 'Submit',
    detail: 'View details',
    loading: 'Loading...',
    language: 'Language',
    japanese: '日本語',
    english: 'English',
  },
  login: {
    title: 'Log in',
    email: 'Email address',
    password: 'Password',
    submit: 'Log in',
    impactedPod: 'Name of the affected pod',
    impactedPodHelper:
      'Logins are handled by the gameday-workflow-user service. Open its APM Summary page and scroll down to the Infrastructure section, then copy the string after \'pod: \'.',
    failed: 'Login failed',
    podSaturated:
      'The server is under heavy load and cannot sign you in. In New Relic, find the pod whose CPU % stays above 50, then enter that pod name.',
    stubMode: 'Stub mode: you can log in with any email address and password',
    welcome: 'Welcome to the workflow management system',
  },
  dashboard: {
    applications: 'My applications',
    applicationsDescription: 'Review the applications you have submitted',
    approvals: 'Pending approvals',
    approvalsDescription: 'Review the applications waiting for your approval',
    companyApplications: 'All applications',
    companyApplicationsDescription: 'Review applications across the company',
    approved: 'Approved applications',
    approvedDescription: 'Review applications that have been approved',
    statistics: 'Statistics',
    statisticsDescription: 'Review statistics about applications',
    manual: 'Manual',
    manualDescription: 'Read how to fill in each application',
    notifications: 'Notifications',
    notificationsDescription: 'Review notifications addressed to you',
    hr: 'HR',
    hrDescription: 'Edit the direct manager of users in your company',
    heading: 'Dashboard',
    totalApplications: 'Total applications',
    pendingApprovals: 'Applications awaiting approval',
    companyApproved: 'Approved applications in your company',
    manualDetail: 'How to fill in each application and how approvals flow',
    loadFailed: 'Failed to load the data',
  },
  applications: {
    newTitle: 'New application',
    type: 'Application type',
    selectPlaceholder: 'Please select',
    typeBusinessTrip: 'Business trip',
    typeVacation: 'Paid leave',
    typeExpense: 'Expense',
    typePromotion: 'Promotion',
    title: 'Title',
    titlePlaceholder: 'Enter a title for this application',
    amount: 'Amount',
    amountPlaceholder: 'Enter the amount',
    departure: 'From',
    arrival: 'To',
    estimatedCost: 'Estimated travel cost',
    estimating: 'Calculating...',
    estimateHint: 'The cost is calculated automatically once you pick both cities',
    estimateDiagnosisTitle: 'Diagnose why the travel cost estimate fails',
    estimateFailed:
      'The request to the travel service timed out and the estimate could not be retrieved. Please try again in a moment.',
    startDate: 'Start date',
    endDate: 'End date',
    days: 'Days',
    daysPlaceholder: 'Enter the number of days',
    daysUnit: 'day(s)',
    daysAuto: 'Calculated automatically: {days} day(s)',
    daysAutoHint: 'This is calculated automatically once you enter both dates',
    dateFormatHint: 'Format: YYYY-MM-DD',
    departureIn: '{days} day(s) until departure.',
    departurePast: 'The departure date is already in the past.',
    twoWeekRule:
      ' This may not satisfy the company rule of applying at least {days} days before departure. Please double-check.',
    description: 'Description',
    descriptionPlaceholder: 'Enter the details of this application',
    descriptionAiHint:
      'Describe the purpose, who you will visit, and what you will do there (an AI reviewer checks this)',
    toConfirm: 'Review',
    confirmTitle: 'Confirm your application',
    confirmBody: 'This is what will be submitted. Check the details and press Submit.',
    submitting: 'Submitting...',
    created: 'The application was created successfully',
    errorAmount: 'Enter a valid amount',
    errorDates: 'Enter both the start date and the end date',
    errorDateOrder: 'The start date must be on or before the end date',
    errorDays: 'Enter the number of days',
    errorNoLogin: 'Login information was not found. Please log in again.',
    errorCreate: 'Could not submit the application',
  },
  hr: {
    usersTitle: 'HR: user list',
    emptyUsers: 'No users',
    name: 'Name',
    email: 'Email address',
    role: 'Role',
    department: 'Department',
    manager: 'Direct manager',
    actions: 'Actions',
    editManager: 'Edit the manager',
    editManagerTitle: 'Edit the direct manager',
    backToList: 'Back to the list',
    unset: 'Not set',
    save: 'Save',
    updated: 'The direct manager has been updated',
    loadUsersFailed: 'Failed to load the users of your company',
    loadUserFailed: 'Failed to load the user',
    updateFailed: 'Failed to update the direct manager',
  },
  workflow: {
    title: 'Approval progress',
    step: 'Step {n}',
    approver: 'Approver',
    statusCompleted: 'Approved',
    statusCurrent: 'Pending',
    statusRejected: 'Rejected',
    statusNotStarted: 'Not started',
    current: 'Currently waiting for approval at step {current} of {total}',
    currentApprover: ' (approver: {name})',
    allDone: '✓ Every step has been approved. The application is approved.',
    rejected: '✗ The application was rejected.',
  },
  detail: {
    title: 'Application details',
    number: 'Application no.: {number}',
    approve: 'Approve',
    reject: 'Reject',
    approveButton: 'Approve',
    rejectButton: 'Reject',
    rageClickHintPrefix: 'Trigger a',
    rageClickTerm: 'rage click',
    rageClickHintSuffix:
      ' on the approve button. (The UI changes once it is detected. Reload and try a few times; collecting and analysing the data takes a moment.)',
    applicationId: 'Application ID',
    history: 'Approval history',
    historyStep: 'Step {n}',
    approverId: 'Approver ID: {id}',
    comment: 'Comment',
    commentWithValue: 'Comment: {comment}',
    receipts: 'Receipt images ({count})',
    notFound: 'The application was not found',
    commentDialogTitle: '{action} - add a comment',
    commentDialogBody: 'Add a comment if you like (optional). You will then see a confirmation.',
    commentLabel: 'Comment (optional)',
    commentPlaceholder: 'Why are you approving or rejecting this?',
    confirmDialogTitle: '{action} - confirm',
    confirmApprove: 'Approve this application?',
    confirmReject: 'Reject this application?',
    processing: 'Processing...',
    loadFailed: 'Failed to load the application',
    approvalFailed: 'The approval could not be processed',
    promotionRestricted: 'Only managers and directors can view promotion applications',
  },
  misc: {
    scoreUpTo: 'UP TO CHAPTER {n}',
    scoreLive: 'LIVE / {n} missions',
    noTeams: 'No team has scored yet.',
    transferScores: 'Send scores',
    transferTitle: 'Send the scores to Dojo?',
    transferBody:
      'This sends the scores of the {n} scoring teams between team {from} and {to} to event {eventId}. Scores already in Dojo are overwritten.',
    transferSubmit: 'Send',
    transferCancel: 'Cancel',
    transferring: 'Sending...',
    transferDone: 'Sent the scores of {n} teams.',
    transferUnmatched: '{n} teams did not match this event and were not saved.',
    transferFailed: 'Could not send the scores: {message}',
    progressBoard: 'Team progress board',
    ofChapters: 'out of {n} chapters',
    receiptAlt: 'Receipt {n}',
    prevImage: 'Previous image',
    nextImage: 'Next image',
    chatTitle: 'AI assistant',
    chatWelcome: 'Ask me anything you like.',
    chatPlaceholder: 'Type a message...',
    chatError: 'Something went wrong. Please try again.',
  },
  manual: {
    title: 'Manual',
    intro: 'These articles explain how to fill in each application and how approvals flow. Read the relevant one before you apply.',
    backToList: 'Back to the manual',
  },
  lists: {
    applicationsTitle: 'Applications',
    approvalsTitle: 'Approvals',
    toDashboard: 'Dashboard',
    newApplication: 'New application',
    empty: 'No applications',
    emptyApprovals: 'No applications are waiting for your approval',
    number: 'Application no.',
    title: 'Title',
    applicationTitle: 'Application title',
    applicationId: 'Application ID',
    type: 'Type',
    applicant: 'Applicant',
    amount: 'Amount',
    period: 'Period',
    status: 'Status',
    step: 'Approval step',
    stepValue: 'Step {current} of {total}',
    nextApprover: 'Next approver',
    createdAt: 'Created',
    updatedAt: 'Updated',
    approvedDone: 'Approved',
    finalApproved: 'Fully approved',
    loadFailed: 'Failed to load the applications',
    loadApprovalsFailed: 'Failed to load the approvals',
    loading: 'Loading...',
    daysSuffix: ' day(s)',
    companyApplicationsTitle: 'All applications',
    approvedTitle: 'Approved applications',
    emptyApproved: 'No approved applications',
    latestComment: 'Latest comment',
    applicationType: 'Application type',
    loadCompanyFailed: 'Failed to load the applications',
    loadApprovedFailed: 'Failed to load the approved applications',
    notificationsTitle: 'Notifications',
    backToDashboard: 'Back to the dashboard',
    emptyNotifications: 'No notifications yet',
    receivedAt: 'Received',
    kind: 'Type',
    subject: 'Subject',
    body: 'Body',
    loadNotificationsFailed: 'Failed to load the notifications',
    notificationApprovalRequest: 'Approval request',
    notificationApproved: 'Approved',
    notificationRejected: 'Rejected',
    notificationSubmitted: 'Submitted',
  },
  quiz: {
    submit: 'Submit',
    searchOrType: 'Search the options or type your own',
    searchOrTypeEnter: 'Search the options, or type your own and press Enter',
    transaction360: {
      heading: 'Understand the CreateApplication transaction of gameday-workflow-frontend',
      intro: 'Submit an application and take a look. Hint: CreateApplication /',
      introTail: 'Review it and answer the four questions below (select every option that applies).',
      correct: 'Correct! You now understand the CreateApplication transaction.',
      correctHint: 'If an application error is showing, check the error details and resolve it too',
      q1: 'Q1. Select every application related to the CreateApplication request',
      q1Label: 'Related applications (multiple)',
      q2: 'Q2. The Transaction-initiating sessions list shows user journeys collected by the Browser agent. Do those journeys always include the action that creates an application? (check about three of them)',
      q2Label: 'Yes / No (pick the one that applies)',
      q3: 'Q3. Select every correct statement you can confirm in the Supporting infrastructure entities panel',
      q3Label: 'Correct statements about the infrastructure (multiple)',
      q4: 'Q4. Select every service that gameday-workflow-frontend calls directly',
      q4Label: 'Directly called services (multiple)',
    },
    rageClick: {
      heading: 'A rage click was detected. Please answer the questions.',
      intro: 'Based on what you found in New Relic, answer the three questions below.',
      correct: 'Correct!',
      q1: 'Q1. Which query was used for the detection?',
      q1Label: 'NRQL query (select or type)',
      q2: 'Q2. After the rage click was detected, which destination was notified successfully?',
      q2Label: 'Notification destination (select or type)',
      q3: 'Q3. Pick the description that best matches how the notification is processed',
      q3Label: 'How the notification is processed (select or type)',
    },
    slowList: {
      heading: 'Diagnose why this screen is slow',
      intro:
        "Use New Relic's Performance Risks Inbox. Based on what you find, answer the three questions below. Hint: gameday-workflow-application-approval",
      correct: 'Correct! You identified the cause.',
      q1: 'Q1. What kind of performance problem is happening?',
      q1Label: 'Type of problem (select or type)',
      q2: 'Q2. Which tables are involved? (multiple)',
      q2Label: 'Table names (multiple)',
      q3: 'Q3. What improvement is being suggested? (Hint: click Generate insights to see the suggestion)',
      q3Label: 'Suggested improvement (select or type)',
    },
  },
  status: {
    approved: 'Approved',
    rejected: 'Rejected',
    pending: 'Pending',
  },
  roles: {
    director: 'Director',
    accounting: 'Accounting',
    manager: 'Manager',
    engineer: 'Engineer',
    hr: 'HR',
  },
  missions: {
    heading: 'Missions',
    hiddenHeading: 'Hidden missions',
    intro:
      'Click a panel to take on that mission. You can work on only one mission at a time, and you cannot switch until you clear it.',
    sealed: 'Take on\nmission {n}',
    sealedSide: 'Take on mission',
    cleared: 'Cleared!!',
    challenging: 'In progress',
    challengers: '{n} team(s) working on it',
    dialogTitleSealed: 'Take on this mission?',
    dialogSealedBody:
      'The mission details are revealed once you start. You can work on only one mission at a time, and you cannot switch until you clear it.',
    challengingNotice:
      'You are working on this mission. Once you clear it, you can pick the next one.',
    startButton: 'Take on this mission',
    startFailed: 'Could not start the mission.',
    startFailedRetry: 'Could not start the mission. Please try again in a moment.',
    reasonAnotherActive: 'Clear the mission you are working on before taking on another one.',
    reasonLocked: 'This mission is not unlocked yet.',
    reasonAlreadyCleared: 'You have already cleared this mission.',
    reasonNotChallengeable: 'This mission cannot be taken on directly.',
    reasonUnknownCompany: 'Could not identify your team. Please log in again.',
  },
  diagnosis: {
    title: 'Diagnose the cause',
    description:
      'Based on what you found in New Relic, pick the cause from the list or type it in directly.',
    inputLabel: 'Cause (select or type)',
    placeholder: 'Search the options or type your own',
    check: 'Check',
    correct: 'Correct! You identified the cause.',
    incorrect: 'Not quite. Investigate in New Relic once more and pick again.',
    notChallenging:
      'You have not taken on this mission yet, so the clear was not recorded. Open this mission from the dashboard and answer again.',
    optionsFailed: 'Failed to load the options',
    checkFailed: 'Failed to check the answer',
  },
  runbook: {
    title: 'Workaround: slow approved application list',
    description:
      'This switches how the list loads related data. It applies only to the company you belong to and does not affect other companies. It lasts for the day and is reset by the daily maintenance.',
    applying: 'Applying...',
    applied:
      'The workaround has been applied. From the next request onward, the approved list makes far fewer database calls. Compare the numbers before and after in New Relic to confirm the effect.',
    already: 'The workaround has already been applied for this company.',
    blocked:
      'The workaround cannot be applied because the investigation is not complete. Finish the "Isolating the cause" section of the runbook first, then open this page again.',
    unauthenticated:
      'You need to log in so we can identify your company. Please log in and open this page again.',
    error: 'Could not apply the workaround. Please try again in a moment.',
    toLogin: 'Go to the login page',
    toDashboard: 'Go to the dashboard',
    retry: 'Try again',
  },
};

export const messages: Record<Locale, Messages> = { ja, en };
